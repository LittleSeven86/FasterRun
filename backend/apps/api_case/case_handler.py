...
# !/usr/bin/env python
# -*- coding:utf-8 -*-
# @FileName  :case_handler.py
# @Time      :2024/10/3 13:34
# @Author    :XiaoQi
import json
import os
import traceback
...
from Runner.loader import load_func_content, load_module_functions
from Runner.parser import Parser
from Runner.steps.step import Step
from Runner.steps.step_api_requet import RunRequestStep
...
from apps.env.dao.envDao import Env, EnvFunc
from apps.systems.dao.fileDao import FileInfo
from common.enum.baseEnum import TStepTypeEnum
from common.model.StepModel import TConfig, TStep, TRequest, TUiRequest, TSqlRequest, TWaitRequest, TIFRequest, \
    TLoopRequest, TScriptRequest, TestCase
from common.utils.generate_rsa_key import decrypt_rsa_password
from config.Config import config
from common.model.StepDataModel import ApiBaseModel as ApiBaseSchema, TStepData, RequestMode, RawLanguageEnum
...
class HandleConfig(object):
    ...
    async def init_functions(self):
        from common.utils import basic_function
...
class HandleStepData(object):
    ...
    async def __init_sql_step(self) -> Step:
        """sql步骤初始化"""
        source_info = await DataSourceDao.get(self.api_info.request.source_id)
    ...
    async def handle_steps(steps: typing.List[typing.Union[TStepData]]) -> typing.List[TStepData]:
        ...
        for step in steps:
            ...
            if step.step_type.lower() == TStepTypeEnum.api.value.lower():
                ...
                api_info = await ApiInfoDao.get(step.source_id, True)
                if api_info:
