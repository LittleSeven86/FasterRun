...
from apps.api_case.model.apiCaseModel import ApiCaseQuery, ApiCaseIdsQuery, ApiCaseIn, ApiCaseStepDataSchema, ApiCaseId, \
    ApiTestCaseRun, TestCaseRun
from apps.api_info.dao.apiInfoDao import ApiCaseStep
from apps.report.model.reportModel import TestReportSaveSchema
...
from common.model.StepDataModel import TStepData
from common.serialize.serialize import default_serialize
from common.utils.asyc_to_converter import sync_to_async
...
class ApiCaseService:
    ...
    async def list(params: ApiCaseQuery) -> typing.Dict:
        """用例列表"""
        result = await ApiCaseDao.get_list(params)
        for data in result:
            if 'include' in data:
    ...
    async def get_case_by_ids(params: ApiCaseIdsQuery) -> typing.Dict:
        """用例列表"""
        result = await ApiCaseDao.get_case_by_ids(ids=params.ids)
        return result
    ...
    async def save_or_update(params: ApiCaseIn) -> typing.Dict:
        """用例保存"""
        # 判断用例名是否重复
        exist_case_info = await ApiCaseDao.get_case_by_name(name=params.name)
        case_info = await ApiCaseDao.get(params.id)
        ...
        if case_info:
            ...
            if exist_case_info:
                raise ParameterError("用例名以存在!")
        data = await ApiCaseDao.create_or_update(params.dict(exclude={"step_data"}))
    ...
    async def deleted(params: ApiCaseId):
        """删除用例"""
        return await ApiCaseDao.delete(params.id)

    @staticmethod
    async def get_case_info(params: ApiCaseId) -> typing.Dict:
        """获取用例信息"""
        api_case = await ApiCaseDao.get(params.id, to_dict=True)
    ...
    async def run_case(params: ApiTestCaseRun):
        ...
        case_info = await ApiCaseDao.get(params.id, to_dict=True)
    ...
    async def get_count_by_user():
        ...
        count_info = await ApiCaseDao.get_count_by_user_id(user_info.get("id", None))
        if not count_info:
            return 0
        if count_info:
    ...
    async def use_case_relation(params: ApiCaseId):
        api_info = await ApiCaseDao.get_api_by_id(params.id)
        if not api_info:
